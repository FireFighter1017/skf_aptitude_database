...
-- reference ID = 
if exists (select * from INFORMATION_SCHEMA.VIEWS where TABLE_NAME='cas_Inspections')
	...
	CASE
		...
		when sa.ALARMMETHOD is null then
			CAST(
					...
					ia.inspectiontext5 + '(' + CASE ia.ALARMLEVEL5 WHEN 2 then 'Correct' when 3 then 'Alerte' when 4 then 'Danger' else 'Aucun' end + ')' AS NVARCHAR)
